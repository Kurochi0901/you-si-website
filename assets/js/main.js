...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if(key === "products-mini"){
    const list = products.filter(p=>groupOf(p)==="mini");
    renderGrid(sortByPriorityDesc(list), "productGrid");
    injectProductSchema(sortByPriorityDesc(list), "小罐專區商品列表｜酉時喝酒");
  }
...
function getOrderPricing(pickup, shipTemp){
  ...
  const itemsTotal = promo.finalTotal; // 折扣後商品金額
...
function openProduct(id){
  ...
  // ✅ 點空白處關閉（保留你原本行為，但更穩）
  //    若你已經有 #lightbox 的 click handler，這段可不加也沒關係。
  // box.onclick = (e)=>{
